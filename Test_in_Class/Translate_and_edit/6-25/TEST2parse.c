...
extern FILE *fp, *fout; //用于指向输入输出文件的指针
...
//插入符号表动作@ name一 defn,t的程序如下：查符号表的过程
int name_def(char * name) {
	int i, es = 0;
	if (vartablep >= maxvartablep)
		return (21); //符号表是否满
	for (i = vartablep - 1; i >= 0; i--) { //查符号表
		if (strcmp(vartable[i].name, name) == 0) {//是否有重复声明变量
			es = 22; //22表示变量重复声明
			break;
		}
	}
	if (es > 0)
		return (es);
	strcpy(vartable[vartablep].name, name);//将内容填入符号表名字域
	vartable[vartablep].address = datap;//更新地址
...
int if_stat() {
	...
	if (strcmp(token, ")"))
		return (es = 6); //少右括号
	label1 = labelp++; //用label1记住条件为假时要转向的标号（执行第一个动作符号内容,）
...
int expression() {
	...
	if (strcmp(token, "ID") == 0) {
		...
		if (strcmp(token2, "=") == 0) {
			...
			fseek(fp, fileadd, 0); //若非丨^则文件指针回到^前的标识符
			printf("%s %s\n", token, token1);
			es = bool_expr();
			if (es > 0)
				return (es);
